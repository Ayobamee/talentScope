...
//   VITE_SUPABASE_URL       = https://nxjpalkrrixrkxhnmmol.supabase.co
...
const computeScore = (talent, okrs, scores) => {
  if (!talent?.okrs?.length) return null;
  const tScores = scores[talent.id] || {};
  let wSum = 0, wTotal = 0;
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setTalents(tRows || []);
        const sm = {};
        for (const r of (sRows||[])) { if(!sm[r.talent_id]) sm[r.talent_id]={}; sm[r.talent_id][r.okr_id]=r.score; }
        setScores(sm);
  ...
  const deleteOkr = async (id) => {
    if (!confirm("Delete this OKR? Scores linked to it will also be removed.")) return;
    setSaving(true);
    await supabase.from("scores").delete().eq("okr_id", id);
    await supabase.from("okrs").delete().eq("id", id);
    setOkrs(okrs.filter((o)=>o.id!==id));
    const ns = Object.fromEntries(Object.entries(scores).map(([tid,s])=>{ const c={...s}; delete c[id]; return [tid,c]; }));
    setScores(ns);
    setSaving(false); showToast("OKR deleted");
  };

  // ── Talent actions ────────────────────────────────────────
  const openNewTalent = () => { setTalentForm({ name:"", role:"", dept:"", okrs:[] }); setTalentModal(true); };
  const saveTalent = async () => {
    if (!talentForm.name.trim()) return;
    setSaving(true);
    const n = { id:uid(), name:talentForm.name, role:talentForm.role, dept:talentForm.dept, okrs:talentForm.okrs };
    await supabase.from("talents").insert(n);
    setTalents([...talents, n]);
    setSaving(false); setTalentModal(false); showToast("Talent added");
  };
  const deleteTalent = async (id) => {
    ...
    const ns={...scores}; delete ns[id]; setScores(ns);
    setSaving(false); showToast("Talent removed");
  };

  // ── Score actions ─────────────────────────────────────────
  const openScore = (tid) => { setTempScores({...(scores[tid]||{})}); setScoreModal(tid); };
  const saveScoreModal = async () => {
    setSaving(true);
    const talent = talents.find((t)=>t.id===scoreModal);
    const rows = talent.okrs.filter((oid)=>tempScores[oid]!==undefined).map((oid)=>({ talent_id:scoreModal, okr_id:oid, score:tempScores[oid] }));
    if (rows.length) await supabase.from("scores").upsert(rows, { onConflict:"talent_id,okr_id" });
    setScores({...scores, [scoreModal]:{...(scores[scoreModal]||{}),...tempScores}});
    setSaving(false); setScoreModal(null); showToast("Scores saved");
  };

  // ── computed ──────────────────────────────────────────────
  const allScored = talents.map((t)=>({...t, score:computeScore(t,okrs,scores)})).filter((t)=>t.score!==null);
  const avg       = allScored.length ? Math.round(allScored.reduce((s,t)=>s+t.score,0)/allScored.length) : null;
  const top       = [...allScored].sort((a,b)=>b.score-a.score)[0];
  const sorted    = [...allScored].sort((a,b)=>sortDir==="desc"?b.score-a.score:a.score-b.score);
  ...
  return (
    <div style={S.app}>
      ...
      <main style={S.main}>
        ...
        {/* DASHBOARD */}
        {tab==="dashboard" && <>
          <div style={S.pageHeader}><div><div style={S.pageTitle}>Overview</div><div style={S.pageSub}>Executive performance intelligence</div></div></div>
          <div style={S.content}>
            <div style={S.grid4}>
              {[
                { label:"Total Talents",  value:talents.length,               sub:`${allScored.length} scored` },
                { label:"Average Score",  value:avg??"—",                     sub:avg?`Grade ${gradeOf(avg)}`:"No scores yet", vc:avg?gradeColor(gradeOf(avg)):"#aeaeb2" },
            ...
            <div style={{ ...S.grid2, marginTop:16 }}>
              <div style={S.card}>
                ...
                <div style={{ fontSize:12, color:"#aeaeb2", marginBottom:16 }}>Talent breakdown by performance band</div>
                {["A","B","C","D","F"].map((g)=>{
                  const cnt=allScored.filter((t)=>gradeOf(t.score)===g).length;
                  const pct=allScored.length?Math.round((cnt/allScored.length)*100):0;
              ...
              <div style={S.card}>
                ...
                <div style={{ fontSize:12, color:"#aeaeb2", marginBottom:16 }}>Comparative team performance</div>
                {DEPARTMENTS.map((d)=>{
                  const sc=talents.filter((t)=>t.dept===d.id).map((t)=>computeScore(t,okrs,scores)).filter((s)=>s!==null);
            ...
            <div style={{ ...S.grid2, marginTop:16 }}>
              <div style={S.card}>
                ...
                <div style={{ fontSize:12, color:"#aeaeb2", marginBottom:16 }}>Highest scoring talents this cycle</div>
                {allScored.length===0 && <div style={{ color:"#aeaeb2", fontSize:13 }}>Score talents to see rankings.</div>}
                {[...allScored].sort((a,b)=>b.score-a.score).slice(0,5).map((t,i)=>(
              ...
              <div style={S.card}>
                ...
                <div style={{ fontSize:12, color:"#aeaeb2", marginBottom:16 }}>All scored talents at a glance</div>
                {allScored.length===0 && <div style={{ color:"#aeaeb2", fontSize:13 }}>No scored talents yet.</div>}
                <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                  {[...allScored].sort((a,b)=>b.score-a.score).map((t)=>{
        ...
        {tab==="talents" && <>
          ...
          <div style={S.content}>
            ...
            <div style={S.grid3}>
              {talents.map((t)=>{
                const score=computeScore(t,okrs,scores); const grade=score!==null?gradeOf(score):null;
                return <div key={t.id} style={{ ...S.card, display:"flex", flexDirection:"column", gap:12 }}>
                  ...
                  <div>
                    <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:"#aeaeb2", marginBottom:5 }}>
                      <span>{t.okrs.length} OKR{t.okrs.length!==1?"s":""}</span>
                      <span style={{ fontWeight:600, color:score!==null?gradeColor(grade):"#aeaeb2" }}>{score!==null?`${score}%`:"Not scored"}</span>
                    </div>
                    <ScoreBar score={score} />
                  </div>
                  <div style={{ display:"flex", gap:8 }}>
                    <button style={{ ...S.btn("secondary"), flex:1, justifyContent:"center" }} onClick={()=>openScore(t.id)}>Score</button>
        ...
        {/* SCORING */}
        {tab==="scoring" && <>
          <div style={S.pageHeader}><div><div style={S.pageTitle}>Score Talents</div><div style={S.pageSub}>Evaluate OKR performance per talent</div></div></div>
          <div style={S.content}>
            ...
            {talents.map((t)=>{
              const score=computeScore(t,okrs,scores); const grade=score!==null?gradeOf(score):null;
              const scored=t.okrs.filter((oid)=>(scores[t.id]||{})[oid]!==undefined).length;
              return <div key={t.id} style={{ ...S.card, marginBottom:12 }}>
                <div style={{ display:"flex", alignItems:"center", gap:14 }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:11, color:"#aeaeb2", marginTop:3 }}>{scored}/{t.okrs.length} OKRs scored</div>
                  </div>
                ...
                {t.okrs.length>0 && <div style={{ marginTop:14, borderTop:"1px solid rgba(0,0,0,0.07)", paddingTop:14, display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))", gap:8 }}>
                  {t.okrs.map((oid)=>{
                    const okr=okrs.find((o)=>o.id===oid); const s=(scores[t.id]||{})[oid]; if(!okr) return null;
        ...
        {tab==="rankings" && <>
          ...
          <div style={S.content}>
            <div style={S.card}>
              {sorted.length===0 && <div style={{ color:"#aeaeb2", fontSize:13, padding:"32px 0", textAlign:"center" }}>No scored talents yet.</div>}
              {sorted.length>0 && <table style={{ width:"100%", borderCollapse:"collapse" }}>
                ...
                <tbody>{sorted.map((t,i)=>{
                  ...
                  const okrCount=t.okrs.filter((oid)=>(scores[t.id]||{})[oid]!==undefined).length;
      ...
      {/* Talent modal */}
      <Modal open={talentModal} onClose={()=>setTalentModal(false)} title="Add Talent"
        footer={<><button style={S.btn("secondary")} onClick={()=>setTalentModal(false)}>Cancel</button><button style={S.btn()} onClick={saveTalent}>{saving?"Saving…":"Save Talent"}</button></>}>
        ...
        <Field label="Department"><Sel value={talentForm.dept} onChange={(v)=>setTalentForm({...talentForm,dept:v,okrs:[]})}><option value="">— Select Department —</option>{DEPARTMENTS.map((d)=><option key={d.id} value={d.id}>{d.name}</option>)}</Sel></Field>
      ...
      {scoreModal && (()=>{
        ...
        return <Modal open={!!scoreModal} onClose={()=>setScoreModal(null)} title={`Score — ${talent.name}`}
